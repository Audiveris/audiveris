...
/**
 * Class <code>SectionView</code> is an abstract class to define common
    ...
    implements VertexView
{
    ...
    /**
     * Assigned color index. This is the permanent default, which is set
     * when the containing {@link LagView} is created, and which is also
     * used when the color is reset by {@link #resetColor}
     */
    protected int colorIndex = -1;

    /**
     * Color currently used. By defaut, the color corresponds to the
     * colorIndex of the palette. But, temporarily, a section can be
     * assigned a different color, for example to highlight the section.
     */
                        ...
                        Zoom zoom)
    {
        ...
        dspPolygon = new Polygon(new int[pointNb],
                                 new int[pointNb],
                                 pointNb);
    ...
    //---------------//
    /**
     * Report the color index assigned to this section view. A -1 value
    ...
    //---------------------//
    /**
     * Determine the proper color index in the palette to assign to this
     ...
     *
     * @param viewIndex the index of this view in the collection of views
     * for this section, which is identical to the index of the containing
     * lag view in the collection of views for the lag.
     */
    public void determineColorIndex (int viewIndex)
    {
        ...
        for (S sct : section.getTargets()) {
            SectionView v = (SectionView) sct.getViews().get(viewIndex);
            int c = v.colorIndex;
            if (c != -1) {
                colorUsed[c] = true;
            }
        ...
        // This should not happen: thanks to a theorem I forgot, we just
        // need 3 colors for areas on a plan.
        if (best == -1) {
     ...
     *
     * @return the properly oriented display rectangle that fits to the
     * section
     */
     ...
     * @return true if the section is concerned by the clipping rectangle,
     * which means if (part of) the section has been drawn
     */
