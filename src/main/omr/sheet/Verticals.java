...
/**
 * Class <code>Verticals</code> is in charge of retrieving all the vertical
 * sticks of all systems in the sheet at hand. Bars are assumed to have
    ...
    implements GlyphDirectory
{
    ...
    private static final Logger logger = Logger.getLogger(Verticals.class);
        ...
        throws ProcessingException
    {
        ...
        for (SystemInfo system : sheet.getSystems()) {
            stemNb += retrieveVerticals(system);
        }
        ...
        logger.info(stemNb + " stem(s) found");
    }
    ...
    // retrieveVerticals //
    //-------------------//
    private int retrieveVerticals (SystemInfo info)
            throws ProcessingException
    {
        // Define the suite of Checks
        StemCheckSuite suite = new StemCheckSuite(info);

        double minResult = constants.minCheckResult.getValue();
        List<Stick> stems = info.getStems();
        List<Stick> sticks = info.getVerticalSticks();
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("Found " + stems.size() + " stems");
        }
        ...
        extends StickView
    {
        ...
        public MyView ()
        {
            super(vLag, null, Verticals.this);
        }
