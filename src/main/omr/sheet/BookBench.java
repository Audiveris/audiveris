...
/**
 * Class {@code BookBench} is in charge of recording all important information related
 * to the processing of a book, and producing an output formatted as "key = value" lines
 * of text.
 * <p>
 * In order to cope with possible multiple recordings with the same radix, we always add to a
 * temporary property set, using numbered suffixes (".01", ".02", etc) so that no data is ever
 * overwritten.
 * The temporary set contains only lines formatted as "radix.suffix = value".
 * <p>
 * When the recordings are about to be flushed, the temporary set is used to produce a clean set of
 * external properties, according to the following rules:<ul>
 * <li>When only the ".01" suffix exists for a given radix, then the externals just contains the
 * "radix = value" line, and the ".01" suffix is not transferred to the output.</li>
 * <li>When more than the .01 suffix exist for a given radix, then these intermediate "radix.suffix
 * = value" pairs are copied to the externals as they are. The last key/value pair is also used to
 * set the "radix = value" pair in the externals (so that the latest value is always accessible
 * through its simple radix)</li>
 * <li>For a special kind of keys (step.[name].duration), the "radix = value" line does not contain
 * the latest intermediate value, but rather the sum of all intermediate values</li></ul>
 * <p>
 * The recorded data can be flushed to disk on specific occasions, to make sure that no data ever
 * get lost even in the case of step cancellation or program interruption.
 * <br/>
 * In case of step cancellation the line "whole.cancelled = true" is added to the externals.
 * <br/>
 * In case of program interruption the line "whole.interrupted = true" is kept in the externals.</p>
 *
        ...
        extends Bench
{
    ...
    private static final Logger logger = LoggerFactory.getLogger(BookBench.class);
    ...
    //--------------//
    /**
     * Build the externals properties, radix by radix, playing with the key suffixes.
     */
