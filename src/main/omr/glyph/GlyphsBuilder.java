...
/**
 * Class <code>GlyphsBuilder</code> is in charge of building glyphs, it handles
 * the gathering of remaining glyphs out of a sheet, as well as the building of
 * compound glyphs.
 *
...
public class GlyphsBuilder
{
     ...
     * impact on either the containing lag, nor the contained sections.
     *
    ...
    public Integer buildInfo ()
    {
        ...
        logger.info(
            (vLag.getLastGlyphId() - firstGlyphId + 1) + " glyph(s) found");
    ...
    public int retrieveSystemGlyphs (SystemInfo system)
    {
        ...
        for (GlyphSection section : system.getVerticalSections()) {
            ...
            if ((!section.isKnown()) &&
                ...
                (section.getGlyph()
                        .getId() < newGlyphId))) {
                Glyph glyph = buildGlyph(system, section);
    ...
    //-------------//
    /**
     * Remove a glyph from the containing system glyph list, as well as from the
     * various spinners of the glyph board if any
     *
