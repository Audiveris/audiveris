    ...
    private void preExpandPackages ()
    {
        for (int row = 0; row < tree.getRowCount(); row++) {
            if (tree.isCollapsed(row)) {
                ...
                Object obj = UnitManager.getInstance().getNode(buildKey(tp));
            ...
            extends DefaultTableCellRenderer
    {
                                                        ...
                                                        int column)
        {
            ...
            if (node == null) { // A Constant row
                Node parent = UnitManager.getInstance().getNode
                        (buildKey(tp.getParentPath()));
                UnitNode unit = (UnitNode) parent;
                ConstantSet set = unit.getConstantSet();
                Constant constant = set.getConstant(tp.getLastPathComponent()
                                                    .toString());
