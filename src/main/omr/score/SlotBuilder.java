...
/**
 * Class {@code SlotBuilder} handles the building of the sequence of slots for
 * every measure within a given system.
 * <p>
 * The key point is to determine when two chords should belong or
    ...
    private class MeasureChecker
    {
                                     ...
                                     Chord ch2)
        {
            ...
            if ((ch1.getStem() != null) && (ch2.getStem() != null)) {
                ...
                if (Math.abs(ch1.getHeadLocation().x - ch2.getHeadLocation().x) > params.maxSlotDx) {
                    return false;
                }
        ...
        private void buildSlots ()
        {
            ...
            List<Chord> actives = new ArrayList<Chord>(measure.getWholeChords());
            ...
            List<Chord> pendings = new ArrayList<Chord>();
            ...
            while (!pendings.isEmpty()) {
                ...
                List<Chord> freeEndings = new ArrayList<Chord>();

                // Chords ending here, with voice not available (beam group)
                List<Chord> endings = new ArrayList<Chord>();

                for (Chord chord : actives) {
                    // Look for chord that finishes at the slot at hand
                    if (!chord.isWholeDuration() && (chord.getEndTime().compareTo(startTime) <= 0)) {
        ...
        {
            Set<Chord> closes = new LinkedHashSet<Chord>();
            closes.add(chord);
                                        ...
                                        Chord ch2)
        {
            ...
            Set<Glyph> seeds = new HashSet<Glyph>();
        ...
        private void inspectBeams ()
        {
            // BeamGroup-based relationships
            for (BeamGroup group : measure.getBeamGroups()) {
                Set<Chord> chordSet = new HashSet<Chord>();

                for (Beam beam : group.getBeams()) {
                    chordSet.addAll(beam.getChords());
                }

                List<Chord> groupChords = new ArrayList<Chord>(chordSet);
        ...
        private void inspectLocations ()
        {
            ...
            List<ChordPair> adjacencies = new ArrayList<ChordPair>();
            int index = 0;
        ...
        private void refineVoices ()
        {
            ...
            List<Chord> firsts = new ArrayList<Chord>();
        ...
        private List<Chord> retrieveIncomingChords (List<Chord> pendings)
        {
            ...
            List<Chord> incomings = new ArrayList<Chord>();
