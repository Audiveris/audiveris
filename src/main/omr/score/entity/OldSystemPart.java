...
/**
 * Class {@code OldSystemPart} handles each of the various parts found in
 ...
 */
public class OldSystemPart
        extends PartNode
{
    //~ Static fields/initializers -----------------------------------------------------------------

    private static final Logger logger = LoggerFactory.getLogger(OldSystemPart.class);

    /** For comparing (TreeNode) OldSystemPart instances according to their id */
    public static final Comparator<TreeNode> idComparator = new Comparator<TreeNode>()
    {
        ...
        public int compare (TreeNode o1,
                            TreeNode o2)
        {
            if (o1 instanceof OldSystemPart && o2 instanceof OldSystemPart) {
                OldSystemPart p1 = (OldSystemPart) o1;
                OldSystemPart p2 = (OldSystemPart) o2;
    ...
    private OldBarline startingBarline;
    ...
    public void addChild (TreeNode node)
    {
        ...
        if (node instanceof OldStaff) {
            staves.addChild(node);
            reset();
        } else if (node instanceof OldMeasure) {
                                  ...
                                  int maxShiftDx)
    {
        for (TreeNode node : getMeasures()) {
            OldMeasure measure = (OldMeasure) node;
     ...
     */
    public void connectSlursWith (OldSystemPart precedingPart)
    {
     ...
     */
    public OldSystemPart createDummyPart (int id)
    {
        logger.debug("{} createDummyPart for id={}", getContextString(), id);

        // Find some concrete system part for the provided id
        OldSystemPart refPart = findRefPart(id);

        OldSystemPart dummyPart = new OldSystemPart(getSystem(), null);
        dummyPart.setId(id);
        dummyPart.setDummy(true);
        dummyPart.setScorePart(refPart.getScorePart());

        OldMeasure nextMeasure = refPart.getFirstMeasure();
        ...
        for (TreeNode mn : getMeasures()) {
            ...
            OldMeasure dummyMeasure = new OldMeasure(dummyPart);
            ...
            for (TreeNode sn : refPart.getStaves()) {
                OldStaff nextStaff = (OldStaff) sn;
                staffIndex++;
                ...
                if (isFirstMeasure) {
                    ...
                    dummyStaff = (OldStaff) dummyPart.getStaves().get(staffIndex);
                }
     ...
     */
    public OldMeasure getFirstMeasure ()
    {
        return (OldMeasure) getMeasures().get(0);
    }
     ...
     */
    public OldStaff getFirstStaff ()
    {
        return (OldStaff) getStaves().get(0);
    }

    //--------------//
    // getFollowing //
    //--------------//
    public OldSystemPart getFollowing ()
    {
     ...
     */
    public OldMeasure getLastMeasure ()
    {
        List<TreeNode> meas = getMeasures();

        if (!meas.isEmpty()) {
            return (OldMeasure) meas.get(meas.size() - 1);
        } else {
            return null;
        }
    }
     ...
     */
    public OldStaff getLastStaff ()
    {
        return (OldStaff) getStaves().get(getStaves().size() - 1);
    }
     ...
     */
    public OldMeasure getMeasureAt (Point point)
    {
        OldMeasure measure = null;

        for (TreeNode node : getMeasures()) {
            measure = (OldMeasure) node;
     ...
     */
    public OldSystemPart getPrecedingInPage ()
    {
...
//        }
     ...
     */
    public OldStaff getStaffJustAbove (Point point)
    {
        OldStaff pointStaff = getStaffAt(point);
        double pitch = pointStaff.pitchPositionOf(point);

        if ((pitch < 0) && (pointStaff != getFirstStaff())) {
            return (OldStaff) pointStaff.getPreviousSibling();
        } else {
            return pointStaff;
        }
    }
     ...
     */
    public OldStaff getStaffJustBelow (Point point)
    {
        OldStaff pointStaff = getStaffAt(point);
        double pitch = pointStaff.pitchPositionOf(point);

        if ((pitch > 0) && (pointStaff != getLastStaff())) {
            return (OldStaff) pointStaff.getNextSibling();
        } else {
            return pointStaff;
        }
    }
     ...
     */
    public OldBarline getStartingBarline ()
    {
    ...
    public void retrieveSlurConnections ()
    {
        ...
        OldSystemPart precedingPart = getPrecedingInPage();
     ...
     */
    public void setStartingBarline (OldBarline startingBarline)
    {
     ...
     */
    private OldSystemPart findRefPart (int id)
    {
        ...
        while (true) {
            ...
            if (nextSystem != null) {
                OldSystemPart part = nextSystem.getPart(id);
        ...
        while (true) {
            ...
            if (prevSystem != null) {
                OldSystemPart part = prevSystem.getPart(id);
