        ...
        implements Vip
{
     ...
     */
    public OldBeamGroup (OldMeasure measure)
    {
    ...
    //----------//
    /**
     * Populate all the OldBeamGroup instances for a given measure.
     *
     * @param measure the containing measure
     */
    public static void populate (OldMeasure measure)
    {
        ...
        if (logger.isDebugEnabled()) {
            ...
            for (OldBeamGroup group : measure.getBeamGroups()) {
                logger.debug("   {}", group);
            }
        }
    ...
    {
        OldChord prevChord = null;

        for (OldChord chord : getChords()) {
            if (prevChord != null) {
                try {
                    // Here we must check for interleaved rest
                    Note rest = OldChord.lookupRest(prevChord, chord);
    ...
    public Rational getDuration ()
    {
        ...
        SortedSet<OldChord> chords = new TreeSet<OldChord>(OldChord.byAbscissa);
     ...
     */
    public OldChord getLastChord ()
    {
        List<OldChord> chords = getChords();
    ...
    //----------//
    /**
     * Report the level of a beam within its containing OldBeamGroup.
     *
    ...
    //-------//
    /**
     * Force all beams (and beam items) to use the same slope within that beam group.
     */
    private void align ()
    {
     ...
     */
    private static SplitOrder checkBeamGroups (OldMeasure measure)
    {
        for (OldBeamGroup group : measure.getBeamGroups()) {
     ...
     */
    private void splitChord (OldChord pivotChord,
                             OldBeamGroup alienGroup)
    {
        logger.debug("Shared : {}", pivotChord);

        // Create a clone of pivotChord (w/o any beam initially)
        OldChord cloneChord = pivotChord.duplicate();
    ...
    private void splitGroup (SplitOrder split)
    {
        ...
        OldBeamGroup chordGroup = new OldBeamGroup(measure);
        ...
        for (Beam aBeam : chordGroup.beams) {
            for (OldChord aChord : aBeam.getChords()) {
     ...
     * Splitting must be separate from browsing to avoid concurrent modification of collections
     */
    private static class SplitOrder
    {
        ...
        final OldBeamGroup group;

        /** A chord of this group, where multiplicity was detected. */
        final OldChord alienChord;

        //~ Constructors ---------------------------------------------------------------------------
        public SplitOrder (OldBeamGroup group,
                           OldChord alienChord)
        {
            this.group = group;
