...
/**
 * Class {@code Articulation} represents an articulation event, a special notation.
        ...
        extends AbstractNotation
{
                                 ...
                                 Point point)
    {
        ...
        if (chord != null) {
            ...
            int dy = Math.min(Math.abs(head.y - point.y), Math.abs(tail.y - point.y));
            double normedDy = measure.getScale().pixelsToFrac(dy);

            if (normedDy <= constants.maxArticulationDy.getValue()) {
                glyph.setTranslation(new Articulation(measure, point, chord, glyph));
        ...
        if (glyph.isVip() || logger.isDebugEnabled()) {
            logger.info("No chord close enough to articulation {}", glyph.idString());
        }
